...
static irqreturn_t kpd_irq_handler_2(int irq, void *dev_id)
{
	/* use _nosync to avoid deadlock */
	disable_irq_nosync(key_irq);
	kpd_print("#####brucelee##### dts key down\n");
	input_report_key(kpd_input_dev, KEY_F24, 1);//KEY_F24
	input_sync(kpd_input_dev);
	kpd_print("#####brucelee##### dts key up\n");	
	input_report_key(kpd_input_dev, KEY_F24, 0);//KEY_VOLUMEUP
	input_sync(kpd_input_dev);
	enable_irq(key_irq);		
	return IRQ_HANDLED;
}
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
	...
	kpd_info("#####brucelee####register dts key IRQ 111\n");
	node = of_find_compatible_node(NULL,NULL,"mediatek, key-eint");
	if(node){
		//of_property_read_u32_array(node, "debounce",ints,ARRAY_SIZE(ints));
		//gpiopin = ints [0];
		//debounce = ints [1];
		key_irq = irq_of_parse_and_map(node, 0);
		r = request_irq(key_irq, kpd_irq_handler_2,IRQF_TRIGGER_NONE , KPD_NAME, NULL);
		kpd_info("#####brucelee####register dts key 222 key_irq = %d\n", key_irq);
		if (r) {
			kpd_info("#####brucelee####register dts key failed (%d)\n", r);
			//misc_deregister(&kpd_dev);
			//input_unregister_device(kpd_input_dev);
			//return r;
		}	
