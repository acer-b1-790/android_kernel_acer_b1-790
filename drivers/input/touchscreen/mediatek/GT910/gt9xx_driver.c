...
static s32 gtp_init_panel(struct i2c_client *client)
{
	...
	GTP_INFO("hxh SENSOR ID:%d", rd_cfg_buf[GTP_ADDR_LENGTH]);
	...
	abs_y_max = (config[RESOLUTION_LOC + 3] << 8) + config[RESOLUTION_LOC + 2];
	int_type = (config[TRIGGER_LOC]) & 0x03;
		GTP_ERROR("abs_x_max=:%d ", abs_x_max);		
		GTP_ERROR("abs_y_max=:%d ", abs_y_max);	
...
#endif
	{
		...
		GTP_DEBUG("hxh X_MAX = %d,Y_MAX = %d,TRIGGER = 0x%02x", abs_x_max, abs_y_max, int_type);
	}
	msleep(20);
out:
	return ret;
...
void gtp_get_chip_type(struct i2c_client *client)
{
	...
	{
		gtp_chip_type = CHIP_TYPE_GT9;
		GTP_ERROR("gtp_chip_type = CHIP_TYPE_GT9");	
	}
	else
	{
		gtp_chip_type = CHIP_TYPE_GT9F;
		GTP_ERROR("gtp_chip_type = CHIP_TYPE_GT9F");	
...
//	node = of_find_matching_node(NULL, touch_of_match);
	...
	if (node) {
		...
		GTP_ERROR("hxh tpd request_irq IRQ touch_irq=%d\n",touch_irq);
...
//			    input_x =720-input_x;
		...
		GTP_ERROR("input_x=:%d ", input_x);		
		GTP_ERROR("input_y=:%d ", input_y);	
				GTP_DEBUG
