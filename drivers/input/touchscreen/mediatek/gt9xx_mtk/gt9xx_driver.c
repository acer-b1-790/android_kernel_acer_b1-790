...
#ifndef CONFIG_GTP_COMPATIBLE_MODE
...
u8 gtp_fw_startup(struct i2c_client *client)
{
		...
		wr_buf[0] = 0xAA;
		wr_buf[1] = 0xAA;
		ret = i2c_write_bytes(client, 0x8041, wr_buf,2);
		if (ret < 0) {
		...
		wr_buf[0] = 0xAA;
		wr_buf[1] = 0xAA;
		i2c_write_bytes(client, 0x8041, wr_buf, 2);
		return SUCCESS;
}
#endif
