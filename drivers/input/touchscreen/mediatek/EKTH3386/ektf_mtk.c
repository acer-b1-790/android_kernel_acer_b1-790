...
*
* 2015.11.1: The releas version 0x0007
...
// for PMIC L8 VGP2 power ++
//#include <platform/upmu_common.h>
#include <mach/upmu_hw.h>
//#include <mt-plat/mt_gpio.h>
//#include <mach/mt_i2c.h>
...
//#define FW_UPDATE_IN_DRIVER // for FW update in driver
...
#define touch_debug(level, ...) \
    do { \
        if (debug >= (level)) \
            TPD_DMESG("[elan] " __VA_ARGS__); \
    } while (0)


uint8_t RECOVERY = 0x00;
int FW_VERSION = 0x00;
int X_RESOLUTION = 3264;
int Y_RESOLUTION = 2048;
//int X_RESOLUTION = 1920;
//int Y_RESOLUTION = 1200;
int FW_ID = 0x00;
int work_lock = 0x00;
int power_lock = 0x00;
...
static unsigned long delay = 8*HZ;
...
struct elan_ktf_ts_data
{
    ...
    struct wake_lock wakelock_gesture;
    // for LCM
    struct regulator *vcc_i2c;
    struct regulator *vdd;
    struct switch_dev touch_sdev; //  for switch device
    u8 gesture;
    u32 intr_gpio_flags;
    u32 rst_gpio_flags;
...
static int elan_ktf_ts_suspend(struct i2c_client *client, pm_message_t mesg); // for early-suspend
#endif
...
#define TPD_OK 0
//	static struct regulator *tp_vgp;
//	/* get LDO supply */
//	static int tp_get_vgp_supply(struct device *dev)
//	{
//		int ret;
//		struct regulator *tp_vgp_ldo;
//		TPD_DMESG(KERN_INFO "[elan] TP: tp_get_vgp_supply is going\n");
//	//	pr_debug("TP: tp_get_vgp_supply is going\n");
//		tp_vgp_ldo = regulator_get(tpd->tpd_dev, "vtouch");
//	//	tp_vgp_ldo = devm_regulator_get(dev, "reg-lcm");
//		if (IS_ERR(tp_vgp_ldo)) {
//			ret = PTR_ERR(tp_vgp_ldo);
//			dev_err(dev, "failed to get reg-tp LDO, %d\n", ret);
//			return ret;
//		}
//	
//		pr_debug("TP: lcm get supply ok.\n");
//	
//		ret = regulator_enable(tp_vgp_ldo);
//		/* get current voltage settings */
//		ret = regulator_get_voltage(tp_vgp_ldo);
//	//	pr_debug("TP LDO voltage = %d in LK stage\n", ret);
//		TPD_DMESG(KERN_INFO "[elan] TP: tp_get_vgp_supply is ok\n");
//		tp_vgp = tp_vgp_ldo;
//	
//		return ret;
//	}
//	
//	int tp_vgp_supply_enable(void)
//	{
//		int ret;
//		unsigned int volt;
//	
//		pr_debug("TP: lcm_vgp_supply_enable\n");
//	
//		if (NULL == tp_vgp)
//			return 0;
//		TPD_DMESG(KERN_INFO "[elan] TP: set regulator voltage lcm_vgp voltage to 3.3V\n");
//	//	pr_debug("TP: set regulator voltage lcm_vgp voltage to 3.3V\n");
//		/* set voltage to 1.8V */
//		ret = regulator_set_voltage(tp_vgp, 3000000, 3000000);
//		if (ret != 0) {
//			pr_err("TP: tp failed to set tp_vgp voltage: %d\n", ret);
//			return ret;
//		}
//	
//		/* get voltage settings again */
//		volt = regulator_get_voltage(tp_vgp);
//		if (volt == 3000000)
//			pr_err("TP: check regulator voltage=3000000 pass!\n");
//		else
//			pr_err("TP: check regulator voltage=3000000 fail! (voltage: %d)\n", volt);
//	
//		ret = regulator_enable(tp_vgp);
//		if (ret != 0) {
//			pr_err("TP: Failed to enable lcm_vgp: %d\n", ret);
//			return ret;
//		}
//	
//		return ret;
//	}
//	
//	int tp_vgp_supply_disable(void)
//	{
//		int ret = 0;
...
//	
//		/* disable regulator */
//		isenable = regulator_is_enabled(tp_vgp);
//	    TPD_DMESG("TP: tp query regulator enable status[0x%d]\n", isenable);
//	//	pr_debug("TP: tp query regulator enable status[0x%d]\n", isenable);
//	
//		if (isenable) {
//			ret = regulator_disable(tp_vgp);
//			if (ret != 0) {
//				pr_err("TP: tp failed to disable lcm_vgp: %d\n", ret);
//				return ret;
//			}
//			/* verify */
//			isenable = regulator_is_enabled(tp_vgp);
//			if (!isenable)
//				pr_err("TP: tp regulator disable pass\n");
//		}
//		return ret;
//	}
...
void elan_ktf_ts_hw_reset()
{
	...
	TPD_DMESG("elan_ktf_ts_hw_reset 0");
    msleep(50);
    gpio_direction_output(ts->rst_gpio, 1);
	TPD_DMESG("elan_ktf_ts_hw_reset 1");	
    msleep(50);	
}
...
static ssize_t write_proc(struct file *file, const char *buf, size_t count, loff_t *data)
{
	...
	if (count == 8) {
		...
		touch_debug(DEBUG_INFO,"---hxh write_proc mIsEnabletwofinger = %d\n",mIsEnabletwofinger);
		touch_debug(DEBUG_INFO,"---hxh write_proc mIsEnablefivefinger = %d\n",mIsEnablefivefinger);
		touch_debug(DEBUG_INFO,"---hxh write_proc mIsEnableDoubleTab = %d\n",mIsEnableDoubleTab);
...
void create_new_proc_entry(void)
{
	//mProc_dir_entry = create_proc_entry(GESTURE_PROC_NAME, 0666, NULL);
      mProc_dir_entry = proc_create(GESTURE_PROC_NAME, 0777, NULL, &wakeup_ops);
        ...
        {
            touch_debug(DEBUG_ERROR,"Couldn't create proc entry!");
        }
        else
        {
            touch_debug(DEBUG_INFO," Create proc entry success!");
...
static ssize_t elan_iap_write(struct file *filp, const char *buff, size_t count, loff_t *offp)
{
    ...
    //if (ret != count) TPD_DMESG("ELAN i2c_master_send fail, ret=%d \n", ret);
    kfree(tmp);
    //return ret;
...
static long ekth3260_ioctl(struct file *file, unsigned cmd, unsigned long arg)
{
    ...
    switch(cmd)
    {
        case TOUCH_IOCTL_FW_CHECK:
            ...
            TPD_DMESG("[elan] %s:[%d]: TOUCH_IOCTL_FW_CHECK. \n", __func__, __LINE__);
        ...
        case TOUCH_IOCTL_FORCE_FW_UPDATE:
            ...
            TPD_DMESG("[elan] %s:[%d]: TOUCH_IOCTL_FORCE_FW_UPDATE. \n", __func__, __LINE__);
        ...
        case TOUCH_IOCTL_READ_CMD:
            ...
            TPD_DMESG("[elan] %s:[%d]: TOUCH_IOCTL_READ_CMD. \n", __func__, __LINE__);

            break;

        case TOUCH_IOCTL_WRITE_CMD:

            TPD_DMESG("[elan] %s:[%d]: TOUCH_IOCTL_WRITE_CMD. \n", __func__, __LINE__);
            ...
            switch(val)
            {
                ...
                default:
                    TPD_DMESG("[elan] %s:[%d]: incorrect val (%d). \n", __func__, __LINE__, val);
            }
            break;

        default:
            TPD_DMESG("[elan] %s:[%d]: incorrect cmd (%d). \n", __func__, __LINE__, _IOC_NR(cmd));
            return -EINVAL;
    }
    return 0;
}
...
static void check_update_flage(struct elan_ktf_ts_data *ts,int New_FW_ID,int New_FW_VER)
{
    int rc;
    printk("[elan] enter %s\n",__func__);
    //int New_FW_ID;
...
#ifdef ONE_FACTORY
    //TPD_DMESG(" [7c16]=0x%02x,  [7c17]=0x%02x, [7c18]=0x%02x, [7c19]=0x%02x\n",  file_fw_data[31766], file_fw_data[31767], file_fw_data[31768], file_fw_data[31769]);
    New_FW_ID = file_fw_data[0x871B] << 8  | file_fw_data[0x871A] ;
    New_FW_VER = file_fw_data[0x850B] << 8  | file_fw_data[0x850A] ;
	TPD_DMESG(" [0x871B]=0x%02x,  [0x871A]=0x%02x, [0x850B]=0x%02x, [0x850A]=0x%02x\n", 
	file_fw_data[0x871B], file_fw_data[0x871A], file_fw_data[0x850B], file_fw_data[0x850A]);
#endif
	...
	if (New_FW_ID   ==  FW_ID)
	{
		...
		{
			TPD_DMESG("hxh1 FNew_FW_VER > FW_VERSION!");
			Update_FW_in_Driver(ts->client);
		}
		else
		{
			TPD_DMESG("hxh2 goto no_update_elan_fw!");
	...
	{
		//Update_FW_in_Driver(ts->client);
		TPD_DMESG("hxh3 FW_ID is different!");
	}
	/*enter recovery*/
	if (0x80 == RECOVERY)  
	{
...
#endif
	msleep(200);
	elan_ktf_ts_rough_calibrate(ts->client);
	return;
...
//IAP_RESTART:
    ...
    for(iPage = 1; iPage <= PageNum; iPage++)
    {
        ...
        for(byte_count = 1; byte_count <= 17; byte_count++)
        {
            if(byte_count != 17)
            {
                //			TPD_DMESG("[ELAN] byte %d\n",byte_count);
                //			TPD_DMESG("curIndex =%d\n",curIndex);
...
PAGE_REWRITE:        
        res = WritePage(szBuff, PageSize);
        printk("[ELAN] WritePage have write %d byte\n ",res);
#endif
    ...
    //elan_ktf_ts_rough_calibrate(private_ts->client);
   // res = __hello_packet_handler(private_ts->client);
   // if(res > 0)
...
int Read_SFR(struct i2c_client *client, int open)
{
    ...
    //count=6;
    	ret = i2c_master_recv(client, voltage_recv, sizeof(voltage_recv));
    	trimtable[count]=voltage_recv[0]<<8 | voltage_recv[1];
    TPD_DMESG("[elan] Open_High_Voltage recv -1 1word =%x %x, trimtable[%d]=%x \n", voltage_recv[0],voltage_recv[1], count, trimtable[count]);
...
int Mass_Erase(struct i2c_client *client)
{
    ...
    TPD_DMESG("[elan] Mass_Erase mass_buff_1=%x %x(a0 00)\n", mass_buff_1[0],mass_buff_1[1]);  // a0 00 : stop
    //add for test
    ...
    ins_write(client, 0xf3);
    	word_scan_out(client);
    	ret = i2c_master_recv(client, mass_buff_1, sizeof(mass_buff_1));
    TPD_DMESG("[elan] Mass_Erase (II) mass_buff_1=%x %x(40 00)\n", mass_buff_1[0],mass_buff_1[1]);  // 40 00
    //add for test
    ...
    */
    ...
    while(finish == 0)
    {
        ...
        {
            mdelay(100);
            //TPD_DMESG("[elan] mass_buff_1[1] >>4  !=1\n");
            i++;
            if(i == 50)
            {
...
int fastmode_write_func(struct i2c_client *client, int j, uint8_t *szBuff)
{
    ...
    //TPD_DMESG("fastmode_write_func, sendfwbuff[0]=0x%x, sendfwbuff[1]=0x%x\n", sendfwbuff[0], sendfwbuff[1]);
    ...
    for(i = 2; i < 129; i = i + 2)  //  1 Page = 64 word, 1 word=2Byte
    {
        ...
        szBuff += 2;
        //TPD_DMESG("[elan] sendfwbuff[%d]=0x%x, sendfwbuff[%d]=0x%x\n", i, sendfwbuff[i], i+1, sendfwbuff[i+1]);
    }
    ...
    //TPD_DMESG("fastmode_write_func, send len=%d (130), Page %d --\n", len, j);
...
int Write_Page(struct i2c_client *client, int j, uint8_t *szBuff)
{
    ...
    while(finish == 0)
    {
        ...
        else
        {
            ...
            TPD_DMESG("[ELAN] read data successfully! buff[0]=0x%x  buff[1]=0x%x  finish=%d\r\n", buff_read_data[0], buff_read_data[1], finish);  //80 10: ok
        }
        if(finish != 1)
        {
            mdelay(10);
            //TPD_DMESG("[elan] Write_Page finish !=1\n");
            i++;
            if(i == 50)
            {
...
int fastmode_read_func(struct i2c_client *client, int j, uint8_t *szBuff)
{
    ...
    //TPD_DMESG("fastmode_write_func, sendfrbuff[0]=0x%x, sendfrbuff[1]=0x%x\n", sendfrbuff[0], sendfrbuff[1]);
    ...
    //TPD_DMESG("fastmode_read_func, recv len=%d (128)\n", len);
...
int normal_read_func(struct i2c_client *client, int j,  uint8_t *szBuff)
{
    ...
    //TPD_DMESG("[elan] Read_Page, j*64=0x%x\n", j*64);
    word_scan_in(client, j * 64);
    ins_write(client, 0x67);
...
int TWO_WIRE_ICE(struct i2c_client *client)
{
    ...
    for(i = 0 ; i < ektSize; i++)
    {
        ...
        curIndex =  curIndex + PageSize;
        //	TPD_DMESG("[Elan] Write_Page %d........................wait\n ", i);
        ...
        //TPD_DMESG("[Elan] Read_Page %d........................wait\n ", i);
        mdelay(1);
        Read_Page(client, i, szBuff);
        //TPD_DMESG("[Elan] Finish  %d  Page!!!!!!!.........wait\n ", i);
    }
...
IAP_RESTART:
    ...
    if(res != sizeof(data))
    {
        TPD_DMESG("[ELAN] dummy error code = %d\n", res);
        touch_debug(DEBUG_ERROR, "[ELAN] dummy error code = %d\n", res);
    }
    mdelay(10);
    // Start IAP
    for(iPage = 1; iPage <= PageNum; iPage++)
    {
        ...
        if(ACK_OK != res)
        {
            ...
            else
            {
                ...
                {
                    TPD_DMESG("[ELAN] ===%d=== page ReStart %d times!\n",  iPage, restartCnt);
                          ...
                          struct device_attribute *attr, char *buf)
{
    ...
    {
        TPD_DMESG("[elan] %s: skip attribute to avoid CTS test (ekth3260_debug = %d)\n", __func__, ekth3260_debug);
...
static ssize_t elan_rst_gpio_store(struct device *dev, struct device_attribute *devattr, const char *buf, size_t count)
{
    ...
    TPD_DMESG("[elan] %s: set rst_pin = %s\n", __func__, (rst_pin > 0 ? "High" : "Low"));
...
static ssize_t elan_rst_gpio_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    TPD_DMESG("[elan] %s: get rst_pin = %d\n", __func__, rst_pin);
                                  ...
                                  struct device_attribute *attr, char *buf)
{
    ...
    TPD_DMESG(KERN_ERR "[Elan] %s: START\n", __func__);
...
RESET_RECV:
    ...
    else
    {
        ...
        if(20==i)
           return 0;
        printk("[ELAN]: reset %d times recv hello\n",i);
        elan_ktf_ts_hw_reset();
        mdelay(200);
        goto RESET_RECV;
    }
   ...
   // }
   // rc = i2c_master_recv(client, buf_recv, 8);
   // TPD_DMESG("[elan] %s: Try Re-Calibration packet %2x:%2x:%2x:%2x\n", __func__, buf_recv[0], buf_recv[1], buf_recv[2], buf_recv[3]);
...
//    uint8_t info_buff_resp[17] = { 0 };
    ...
    if(rc < 0)
    {
        TPD_DMESG("Get Firmware version error\n");
    }
    ...
    if(rc < 0)
    {
        TPD_DMESG("Get Firmware ID error\n");
    }
    major = ((buf_recv[1] & 0x0f) << 4) | ((buf_recv[2] & 0xf0) >> 4);
    minor = ((buf_recv[2] & 0x0f) << 4) | ((buf_recv[3] & 0xf0) >> 4);
    ...
    if(rc < 0)
    {
        TPD_DMESG("Get Bootcode version error\n");
    }
    major = ((buf_recv[1] & 0x0f) << 4) | ((buf_recv[2] & 0xf0) >> 4);
    minor = ((buf_recv[2] & 0x0f) << 4) | ((buf_recv[3] & 0xf0) >> 4);
...
//	Y_RESOLUTION = 1200;	
    ...
    for(i = 0; i < 3; i++)
    {
        ...
        if(info_buff_resp[0] == 0x9b)
        {
            ...
            TPD_DMESG(KERN_INFO "[elan] %s: read resolution from touch FW.\n", __func__);
            break;
        }
        else
        {
            if(i >= 2)
            {
                TPD_DMESG(KERN_INFO "[elan] %s: set resolution directly by driver.\n", __func__);
                break;
            }
            else
                TPD_DMESG(KERN_INFO "[elan] %s: retry get resolution info.\n", __func__);
        }
    }
#endif

    TPD_DMESG(KERN_INFO "[elan] %s: Firmware version: 0x%4.4x\n",
              __func__, ts->fw_ver);
    TPD_DMESG(KERN_INFO "[elan] %s: Firmware ID: 0x%4.4x\n",
              __func__, ts->fw_id);
    TPD_DMESG(KERN_INFO "[elan] %s: Bootcode Version: 0x%4.4x\n",
              __func__, ts->bc_ver);
    TPD_DMESG(KERN_INFO "[elan] %s: x resolution: %d, y resolution: %d\n",
...
static int elan_ktf_ts_setup(struct i2c_client *client)
{
  ...
  //      rc = __fw_packet_handler(client);
   //     if(rc < 0)
   //         TPD_DMESG("[elan] %s, fw_packet_handler fail, rc = %d", __func__, rc);
   //     dev_dbg(&client->dev, "[elan] %s: firmware checking done.\n", __func__);
        //Check for FW_VERSION, if 0x0000 means FW update fail!
   //     if(FW_VERSION == 0x00)
   //     {
   //         rc = 0x80;
    //        TPD_DMESG("[elan] FW_VERSION = %d, last FW update fail\n", FW_VERSION);
   //     }
  //  }
...
static int elan_ktf_ts_recv_data(struct i2c_client *client, uint8_t *buf, int bytes_to_recv)
{
    ...
    //TPD_DMESG("[elan] Elan protocol rc = %d \n", rc);
    if(rc != bytes_to_recv)
    {
    ...
    if(rc != 8)
    {
        TPD_DMESG("[elan] Read the first package error.\n");
        mdelay(30);
        return -1;
    }
    TPD_DMESG("[elan_debug] %x %x %x %x %x %x %x %x\n", buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6], buf[7]);
    mdelay(1);
    ...
    if(buf[0] == 0x6D)      //for five finger
    {
        ...
        if(rc != 8)
        {
            TPD_DMESG("[elan] Read the second package error.\n");
            mdelay(30);
            return -1;
        }
        ...
        if(rc != 2)
        {
            TPD_DMESG("[elan] Read the third package error.\n");
            mdelay(30);
            return -1;
        }
        mdelay(1);
        TPD_DMESG("[elan_debug] %x %x \n", buf[16], buf[17]);
    }
#endif
    //TPD_DMESG("[elan_debug] end ts_work\n");
    return rc;
}
            ...
            for(i = 0; i < finger_num; i++)
            {
                ...
                if(active || mTouchStatus[i])
                {
                    ...
                    if(active)
                    {
                        ...
                        if(debug > 5)TPD_DMESG("[elan] finger id=%d x=%d y=%d size=%d press=%d \n", i, x, y, touch_size, pressure_size); // 
...
static void elan_ktf_ts_report_data(struct i2c_client *client, uint8_t *buf)
{
    ...
    switch(buf[0])
    {
        ...
        case TEN_FINGERS_PKT:
            ...
            {
                //dev_dbg(&client->dev, "[elan] --->>hxh  %d fingers\n", num);
                input_report_key(idev, BTN_TOUCH, 1);
                for(i = 0; i < finger_num; i++)
                {
                    if((fbits & 0x01))
                    {
                        ...
                        //x = X_RESOLUTION-x;
                       // y = Y_RESOLUTION - y;
                        //TPD_DMESG("[elan_debug] %s, x=%x, y=%x\n",__func__, x , y);
                       // if(ts->x_resolution< 0 || ts->y_resolution < 0){
							
				//		ts->x_resolution = 3264;
				//		ts->y_resolution = 2048;
                       //}
						
                       // x = x * 1920  / ts->x_resolution;
                        //y = y * 1200 / ts->y_resolution;
                        ...
                        input_mt_sync(idev);
                        reported++;
                        //if(debug > 5)TPD_DMESG("[elan] finger id=%d x=%d y=%d \n", i, x, y); 
                        if(unlikely(gPrint_point))
...
static int touch_event_handler(void *unused)
{
    ...
    printk("hxh enter interrupt touch_event_handler\n");
    do
    {
      	//enable_irq(private_ts->client->irq);
        ...
        tpd_flag = 0;
		//printk("###%s wait_event_interruptible(waiter, tpd_flag != 0);\n", __func__);
		...
		if (suspend_gesture == true) {
			...
			//printk("###[elan james debug add wake lock timeout at wakelock_gesture %s\n", __func__);
    		wake_lock_timeout(&private_ts->wakelock_gesture, 2*HZ);
			
			set_current_state(TASK_INTERRUPTIBLE);
			wait_event_interruptible(waiter_resume, tpd_halt == 0);
			set_current_state(TASK_RUNNING);
			
		//	printk("[elan james] %s wait i2c ready delay 200m sec\n",__func__);
		//	mdelay(500);
		}

		//printk("###%s before elan_ktf_ts_recv_data\n", __func__);
		
        rc = elan_ktf_ts_recv_data(private_ts->client, buf, PACKET_SIZE);
        if (rc < 0)
        {
            printk("[elan] %s elan_ktf_ts_recv_data rc<0\n",__func__);
            continue;
        }
		
		//printk("###%s after elan_ktf_ts_recv_data\n", __func__);
...
static void elan_ktf_ts_double_click_report(struct i2c_client *client, uint16_t x, uint16_t y, int active)
{
    ...
    if(active)
    {
        ...
        input_report_abs(idev, ABS_MT_POSITION_X, x);
        input_report_abs(idev, ABS_MT_POSITION_Y, y);
        TPD_DMESG("[elan] %s: finger id=%d x=%d y=%d \n", __func__, 0, x, y);
    }
    ...
    if(active == 0)
    {
        ...
        TPD_DMESG("[elan] %s: dobule click up \n", __func__);
    }
    input_sync(idev);
}
#endif
...
static void elan_ktf_ts_gesture_report_data(struct i2c_client *client, uint8_t *buf)
{
    ...
    TPD_DMESG("[elan] %s: ts->gesture = 0x%02x\n", __func__, ts->gesture);
    switch(buf[1])
    {
        case ELAN_GESTURE_DOUBLE_CLICK:
            if(ts->gesture & GESTURE_DOUBLE_CLICK)
            {
                ...
                input_sync(ts->input_dev);
#endif
                TPD_DMESG("[elan] %s: GESTURE_DOUBLE_CLICK %x\n", __func__, buf[1]);
            }
            else
                TPD_DMESG("[elan] %s: SKIP.. GESTURE_DOUBLE_CLICK %x\n", __func__, buf[1]);
            break;
        ...
        case ELAN_GESTURE_W:
            if(ts->gesture & GESTURE_W)
            {
                ...
                input_sync(ts->input_dev);
#endif
                TPD_DMESG("[elan] %s: GESTURE_W %x\n", __func__, buf[1]);
            }
            else
                TPD_DMESG("[elan] %s: SKIP.. GESTURE_W %x\n", __func__, buf[1]);
            break;
        ...
        case ELAN_GESTURE_S:
            if(ts->gesture & GESTURE_S)
            {
                ...
                input_sync(ts->input_dev);
#endif
                TPD_DMESG("[elan] %s: GESTURE_S %x\n", __func__, buf[1]);
            }
            else
                TPD_DMESG("[elan] %s: SKIP.. GESTURE_S %x\n", __func__, buf[1]);
            break;
        ...
        case ELAN_GESTURE_E:
            if(ts->gesture & GESTURE_E)
            {
                ...
                input_sync(ts->input_dev);
#endif
                TPD_DMESG("[elan] %s: GESTURE_E %x\n", __func__, buf[1]);
            }
            else
                TPD_DMESG("[elan] %s: SKIP.. GESTURE_E %x\n", __func__, buf[1]);
            break;
        ...
        case ELAN_GESTURE_C:
            if(ts->gesture & GESTURE_C)
            {
                ...
                input_sync(ts->input_dev);
#endif
                TPD_DMESG("[elan] %s: GESTURE_C %x\n", __func__, buf[1]);
            }
            else
                TPD_DMESG("[elan] %s: SKIP.. GESTURE_C %x\n", __func__, buf[1]);
            break;
        ...
        case ELAN_GESTURE_Z:
            if(ts->gesture & GESTURE_Z)
            {
                ...
                input_sync(ts->input_dev);
#endif
                TPD_DMESG("[elan] %s: GESTURE_Z %x\n", __func__, buf[1]);
            }
            else
                TPD_DMESG("[elan] %s: SKIP.. GESTURE_Z %x\n", __func__, buf[1]);
            break;
        ...
        case ELAN_GESTURE_V:
            if(ts->gesture & GESTURE_V)
            {
                ...
                input_sync(ts->input_dev);
#endif
                TPD_DMESG("[elan] %s: GESTURE_V %x\n", __func__, buf[1]);
            }
            else
                TPD_DMESG("[elan] %s: SKIP.. GESTURE_V %x\n", __func__, buf[1]);
            break;

        default:
            TPD_DMESG("[elan] %s: unknow packet %x\n", __func__, buf[1]);
            return;
    }
    return;
}
...
static void elan_ktf_ts_check_work_func(struct work_struct *work)
{
    ...
    TPD_DMESG(KERN_EMERG "%s, chip may crash, we need to reset it \n", __func__);
    ...
    if(res != 0)
    {
        TPD_DMESG(KERN_INFO "Receive hello package fail\n");
    }
...
static void elan_ktf_ts_work_func(struct work_struct *work)
{
    ...
    #if 1 /* 2016.3.5 james  */
//if (suspend_gesture == true)
    printk("[elan james debug %s\n", __func__);
    wake_lock_timeout(&private_ts->wakelock_gesture, 2*HZ);

#endif /* 2016.3.5 end james */    

//    wake_lock(&private_ts->wakelock);
    /*
    if (gpio_get_value(ts->intr_gpio))
    {
    	TPD_DMESG("[elan] Detected the jitter on INT pin.\n");
    	return IRQ_HANDLED;
    }
    */
...
//		return;
    ...
    if(rc < 0)
    {
        ...
        printk("\n");
        //  for testtest --
        TPD_DMESG("[elan] Received the packet Error.\n");
        enable_irq(ts->client->irq);
//        wake_unlock(&private_ts->wakelock);
        return;
    }
    //TPD_DMESG("[elan_debug] %2x,%2x,%2x,%2x,%2x,%2x,%2x,%2x ....., %2x\n",buf[0],buf[1],buf[2],buf[3],buf[4],buf[5],buf[6],buf[7],buf[17]);
...
static irqreturn_t elan_ktf_ts_irq_handler(int irq, void *dev_id)
{
    ...
    uint8_t buf[PACKET_SIZE] = { 0 };
    // for hello package in gesture mode ++
...
//if(bflag==false)
{
    ...
    if(rc < 0)
    {
        ...
        do
        {
			TPD_DMESG("[elan] hxh Received the packet recv_retry50///.\n");
            //if(debug > 20)TPD_DMESG("[elan] ////////////////////		RETRY %d times !!\n", recv_retry);
           ...
           // else
            ...
            {   
                ...
                if(debug > 20)TPD_DMESG("[elan] RETRY %d: ", recv_retry);
                for(i = 0; i < PACKET_SIZE; i++)
                {
                    if(debug > 20)TPD_DMESG("%2x, ", buf[i]);
                }
                if(debug > 20)TPD_DMESG("\n");

                if(debug > 20)TPD_DMESG("[elan] RETRY %d Received the packet Error.\n", recv_retry);
                recv_retry++;
            }
            else
            {
                TPD_DMESG("[elan] RETRY %d times pass.\n", recv_retry);
                break;
            }
    ...
    if(buf[0] == HSYNC_PKT)
    {
        if(hsync_count >= 1000)
            ...
            hsync_count++;
        TPD_DMESG("[elan] %s: into HSYNC count = %d\n", __func__, hsync_count);
        enable_irq(ts->client->irq);
        wake_unlock(&private_ts->wakelock);
        return IRQ_HANDLED;
    }
    ...
    //  for g package in gesture mode ++
#if HELLOW_IN_GESTURE_WORKAROUND
    //TPD_DMESG("[elan] %s: gesture mode check ...\n", __func__);
            ...
            && ((buf[0] == 0x66 && buf[1] == 0x66 && buf[2] == 0x66 && buf[3] == 0x66)))
    {
        ...
        if(ret < 0)
            TPD_DMESG("[elan] %s: re-send gesture_wake_up_cmd fail\n", __func__);

        msleep(2);

        ret = i2c_master_send(ts->client, gesture_mode_check_cmd, sizeof(gesture_mode_check_cmd));
        if(ret < 0)
            TPD_DMESG("[elan] %s: re-send gesture_mode_check_cmd fail\n", __func__);

        msleep(2);

        rc = i2c_master_recv(ts->client, buf_recv, 4);
        if(rc != 4)
            TPD_DMESG("[elan] %s: re-read gesture mode state error.\n", __func__);

        TPD_DMESG("[elan] %s: re-enter gesture mode check: %2x:%2x:%2x:%2x\n", __func__, buf_recv[0], buf_recv[1], buf_recv[2], buf_recv[3]);
...
static void tpd_eint_interrupt_handler(void)
{
	...
	//TPD_DMESG("[elan]   %s TP interrupt has been triggered\n", __func__);
...
static int elan_ktf_ts_register_interrupt(struct i2c_client *client)
{
    ...
    int err = 0;
#if 0
    err = request_threaded_irq(client->irq, NULL, (irq_handler_t)tpd_eint_interrupt_handler,
                               IRQF_TRIGGER_FALLING /*| IRQF_TRIGGER_LOW*/ | IRQF_ONESHOT,
                              client->name, ts);
#else
   err = request_irq(client->irq,  (irq_handler_t)tpd_eint_interrupt_handler,
                               IRQF_TRIGGER_FALLING /*| IRQF_TRIGGER_LOW*/ | IRQF_ONESHOT,
                              client->name, ts);
#endif
    if(err)
                                ...
                                unsigned long event, void *data)
{
    ...
    TPD_DMESG("%s fb notifier callback\n", __func__);
...
static int eKTH3260_parse_dt(struct device *dev, struct elan_ktf_ts_data *ts)
{
    ...
    TPD_DMESG("[elan] %s: ts->rst_gpio_gpio=%d, ts->intr_gpio=%d,  ts->power_gpio=%d, ts->lcmid0_gpio=%d, ts->lcmid2_gpio=%d\n",
           __func__, ts->rst_gpio, ts->intr_gpio, ts->power_gpio, ts->lcmid0_gpio, ts->lcmid2_gpio); // for testtest
    */
    TPD_DMESG("[elan] %s: ts->rst_gpio_gpio = %d, ts->intr_gpio = %d, ts->power_gpio = %d\n", __func__, ts->rst_gpio, ts->intr_gpio, ts->power_gpio);
    return 0;
}
...
static void elan_i2c_set(struct work_struct *work)
{
    ...
    if(ret < 0)
        TPD_DMESG("[elan] %s: send AC cmd fail\n", __func__);
    return;
}

int eKTH3260_cable_status_handler(int state)
{
    TPD_DMESG("[elan] %s: cable state = %d\n", __func__, state);

    switch(state)
    {
        case TYPE_NONE:
            TPD_DMESG("[elan] %s: TYPE_NONE\n", __func__);
            memcpy(AC_cmd, AC_disable_cmd, sizeof(AC_disable_cmd));

            if(private_ts->system_suspend)
                TPD_DMESG("[elan] %s: Skip cable status notifier when system suspend\n", __func__);
            else
        ...
        default:
            TPD_DMESG("[elan] %s: wrong cable type ..\n", __func__);
            break;
    }
    return 0;
}
...
static ssize_t eKTH3260_proc_gesture_write(struct file *filp, const char __user *buf, size_t count, loff_t *off)
{
    ...
    TPD_DMESG("[elan] %s: GESTURE_DOUBLE_CLICK = %c, GESTURE_ENABLE = %c, GESTURE_W = %c, GESTURE_S = %c, GESTURE_E = %c, GESTURE_C = %c, GESTURE_Z = %c, GESTURE_V = %c \n",
              __func__, buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6], buf[7]);

    if((buf[0] == '0') && (buf[1] == '0'))
    {
        ts->gesture = (ts->gesture & (~GESTURE_ENABLE));
        TPD_DMESG("%s: Gesture Disable (ts->gesture = 0x%02x)\n", __func__, ts->gesture);
    }
    else
    {
        ...
        ts->gesture = (ts->gesture | GESTURE_ENABLE);
        TPD_DMESG("%s: Gesture Enable (ts->gesture = 0x%02x) \n", __func__, ts->gesture);
    }
...
static void eKTH3260_create_proc_gesture_file(void)
{
    ...
    if(eKTH3260_proc_gesture_file)
        TPD_DMESG("%s:[%d]: proc Gesture file create sucessed\n", __func__, __LINE__);
    else
        TPD_DMESG("%s:[%d]: proc Gesture file create failed\n", __func__, __LINE__);
    return;
}
...
static void eKTH3260_remove_proc_gesture_file(void)
{
    ...
    TPD_DMESG("%s:[%d]: proc Gesture file removed.\n", __func__, __LINE__);
...
{
    TPD_DMESG("[elan] %s: debug = %d\n", __func__, debug);
    TPD_DMESG("[elan] %s: ekth3260_debug = %d\n", __func__, ekth3260_debug);
    return 0;
}
...
static void eKTH3260_create_logtool_file(void)
{
    ...
    if(eKTH3260_proc_gesture_file)
        TPD_DMESG("%s:[%d]: proc log tool file create sucessed\n", __func__, __LINE__);
    else
        TPD_DMESG("%s:[%d]: proc lgo tool file create failed\n", __func__, __LINE__);
    return;
}
...
{
    extern struct proc_dir_entry proc_root;
    TPD_DMESG("%s:[%d]: proc log tool file removed.\n", __func__, __LINE__);
...
static ssize_t eKTH3260_proc_tp_debug_read(struct file *file, char __user *page, size_t size, loff_t *ppos)
{
    ...
    msleep(30);
    TPD_DMESG(KERN_ERR "[elan] %s: into HSYNC count  = %d\n", __func__, hsync_count);
    TPD_DMESG(KERN_ERR "[elan] %s: ekth3260_debug = %d\n", __func__, ekth3260_debug);
    return 0;
}
...
//	     pmic_set_register_value(PMIC_RG_VGP1_EN, 0); //  for PMIC L8 VGP2 power ++
//		tp_vgp_supply_disable();
		ret = regulator_disable(tpd->reg);
            TPD_DMESG("[elan] %s: set power_pin. = %s\n", __func__, "Low");
        }
        else if(buf[9] == '1')
        {
            gpio_direction_output(ts->power_gpio, 1);
//            pmic_set_register_value(PMIC_RG_VGP1_EN, 1); //  for PMIC L8 VGP2 power ++
//		tp_vgp_supply_enable();
    ...
    {
        TPD_DMESG("[elan] %s: debug_function = testtest \n", __func__);
        if(buf[8] == '1')
        {
            TPD_DMESG("%s: report gesture KEY_POWER test. \n", __func__);
            input_report_key(ts->input_dev, KEY_POWER, 1);
            input_sync(ts->input_dev);
            input_report_key(ts->input_dev, KEY_POWER, 0);
            input_sync(ts->input_dev);
        ...
        {
            TPD_DMESG("[elan] %s: report gesture KEY_POWER2 test. \n", __func__);
        ...
        {
            TPD_DMESG("[elan] %s: report gesture Double Click test. \n", __func__);
            input_report_key(ts->input_dev, KEY_F24, 1);
            input_sync(ts->input_dev);
            input_report_key(ts->input_dev, KEY_F24, 0);
            input_sync(ts->input_dev);
        ...
        {
            TPD_DMESG("[elan] %s: report gesture W test. \n", __func__);
            input_report_key(ts->input_dev, KEY_F23, 1);
            input_sync(ts->input_dev);
            input_report_key(ts->input_dev, KEY_F23, 0);
            input_sync(ts->input_dev);
        ...
        {
            TPD_DMESG("[elan] %s: report gesture S test. \n", __func__);
            input_report_key(ts->input_dev, KEY_F22, 1);
            input_sync(ts->input_dev);
            input_report_key(ts->input_dev, KEY_F22, 0);
            input_sync(ts->input_dev);
        ...
        {
            TPD_DMESG("[elan] %s: report gesture e test.", __func__);
            input_report_key(ts->input_dev, KEY_F21, 1);
            input_sync(ts->input_dev);
            input_report_key(ts->input_dev, KEY_F21, 0);
            input_sync(ts->input_dev);
        ...
        {
            TPD_DMESG("[elan] %s: report gesture C test. \n", __func__);
            input_report_key(ts->input_dev, KEY_F20, 1);
            input_sync(ts->input_dev);
            input_report_key(ts->input_dev, KEY_F20, 0);
            input_sync(ts->input_dev);
        ...
        {
            TPD_DMESG("[elan] %s: report gesture V test. \n", __func__);
            input_report_key(ts->input_dev, KEY_F18, 1);
            input_sync(ts->input_dev);
            input_report_key(ts->input_dev, KEY_F18, 0);
            input_sync(ts->input_dev);
...
static void eKTH3260_create_proc_tp_debug_file(void)
{
    ...
    if(eKTH3260_proc_tp_debug_file)
        TPD_DMESG("[elan] %s:[%d]: proc TP debug file create sucessed\n", __func__, __LINE__);
    else
        TPD_DMESG("[elan] %s:[%d]: proc TP debug file create failed\n", __func__, __LINE__);
    return;
}
...
{
    extern struct proc_dir_entry proc_root;
    TPD_DMESG("[elan] %s:[%d]: proc debug file removed.\n", __func__, __LINE__);
...
{
    if(debug > 20)TPD_DMESG("[elan] %s: start \n", __func__);
    return;
}
#endif
//  for testtest --
...
static void elan_ic_init_work(struct work_struct *work)
{
   ...
   // mdelay(300);
   // disable_irq(private_ts->client->irq);
    rc = elan_ktf_ts_poll(private_ts->client);
    if(rc < 0)
    {
        TPD_DMESG("[elan] %s: Int poll failed!\n", __func__);
    }
    rc = i2c_master_recv(private_ts->client, buf_recv, 8);
    TPD_DMESG("[elan] %s: Try Re-Calibration packet %2x:%2x:%2x:%2x\n", __func__, buf_recv[0], buf_recv[1], buf_recv[2], buf_recv[3]);
    ...
    printk("[ELAN] read FW MSG\n");
     __fw_packet_handler(private_ts->client);
    //enable_irq(private_ts->client->irq);
    }
#if defined IAP_PORTION	
	check_update_flage(private_ts,id,ver);
#endif
...
{    int ret;
    ...
    TPD_DMESG("////////////////////		elan_ktf_ts_probe START v30 \n"); //  for testtest

    // for skip COS/POS ++
    //if(entry_mode==4)
    //{
    //    TPD_DMESG("%s:[%d]: In COS, Skip Probe\n", __func__, __LINE__);
    //    return 0;
    //}
    //else if(entry_mode==3)
    //{
    //    TPD_DMESG("%s:[%d]: In POS, Skip Probe\n", __func__, __LINE__);
    //    return 0;
    //}
    //  for skip COS/POS --
    ...
    {
        TPD_DMESG(KERN_ERR "[elan] %s: i2c check functionality error\n", __func__);
        err = -ENODEV;
    ...
    if(ts == NULL)
    {
        TPD_DMESG(KERN_ERR "[elan] %s: allocate elan_ktf_ts_data failed\n", __func__);
        err = -ENOMEM;
    ...
    {
        TPD_DMESG("int data %d, %d, client->irq %d\n", value[0], value[1], client->irq);
    }
    ...
    if(!ts->elan_irq_wq)
    {
        TPD_DMESG(KERN_ERR "[elan] %s: create elan_irq_wq workqueue failed\n", __func__);
        err = -ENOMEM;
    }
    ...
    if(!ts->elan_wq)
    {
        TPD_DMESG(KERN_ERR "[elan] %s: create elan_wq workqueue failed\n", __func__);
    }
    ...
    if(!ts->elan_ac_wq)
    {
        TPD_DMESG(KERN_ERR "[elan] %s: create elan_ac_wq workqueue failed\n", __func__);
    }
    INIT_DELAYED_WORK(&ts->ac_delay_work, elan_i2c_set);

    TPD_DMESG("[elan] %s: eKTH3260_parse_dt ...\n", __func__); 
    err = eKTH3260_parse_dt(&client->dev, ts);

    TPD_DMESG("[elan] %s: gpio request ...\n", __func__); 
    /*init POWER pin*/
//    err = gpio_request(ts->power_gpio, "ElanTouch-power");
//    if(err < 0)
//        TPD_DMESG(KERN_ERR "[elan] %s:Failed to request GPIO%d (ElanTouch-power) error=%d\n", __func__, ts->power_gpio, err);

//    err = gpio_direction_output(ts->power_gpio, 1);
//    if(err)
//    {
//        TPD_DMESG(KERN_ERR "[elan] %s:Failed to set reset direction, error=%d\n", __func__, err);
...
//		tp_vgp_supply_enable();
    ...
    ret = regulator_enable(tpd->reg);	
    //TPD_DMESG(KERN_INFO "[elan] %s touch power enable\n", __func__);
    msleep(100);

    /*init INTERRUPT pin*/
    err = gpio_request(ts->intr_gpio, "ElanTouch-irq");
    if(err < 0)
        TPD_DMESG(KERN_ERR "[elan] %s:Failed to request GPIO%d (ElanTouch-interrupt) error=%d\n", __func__, ts->intr_gpio, err);

    err = gpio_direction_input(ts->intr_gpio);
    if(err)
    {
        TPD_DMESG(KERN_ERR "[elan] %s:Failed to set intr_gpio direction, error=%d\n", __func__, err);
    ...
    if(err < 0)
        TPD_DMESG(KERN_ERR "[elan] %s:Failed to request GPIO%d (ElanTouch-rst) error=%d\n", __func__, ts->rst_gpio, err);		  	  
    err = gpio_direction_output(ts->rst_gpio, 1);
    if(err)
    {
        TPD_DMESG(KERN_ERR "[elan] %s:Failed to set rst_gpio direction, error=%d\n", __func__, err);
    ...
    */
    ...
    wake_lock_init(&ts->wakelock_gesture, WAKE_LOCK_SUSPEND, "elan-gesture");

    TPD_DMESG("[elan] %s: reset IC ...\n", __func__); //  for testtest
    elan_ktf_ts_hw_reset();
    ...
    if(fw_err < 0)
    {
        TPD_DMESG(KERN_INFO "No Elan chip inside\n");
        //fw_err = -ENODEV;
    }

    //  for set resolution directly ++
    X_RESOLUTION = 3264;
    Y_RESOLUTION = 2048;
    //  for set resolution directly --
...
#if (ACER_GESTURE_WAKEUP)
    input_set_abs_params(ts->input_dev, ABS_MT_TRACKING_ID, 0, MAX_TRACKINGID, 0, 0);
#endif		
	...
	thread = kthread_run(touch_event_handler, 0, "mtk-tpd");	
	if (IS_ERR(thread)) {
		TPD_DMESG("[elan] %s: hxh touch_event_handler failed!! \n", __func__);
		return -1;
	}	
    ...
    if(misc_register(&ts->firmware) < 0)
        TPD_DMESG("[elan] %s: misc_register failed!! \n", __func__);
    else
        TPD_DMESG("[elan] %s: misc_register finished!! \n", __func__);
    // End Firmware Update
    ...
    if(err < 0)
    {
        TPD_DMESG("[elan] %s: [%d]: misc_register failed (%d) \n", __func__, __LINE__, err);
    }
    //  --
...
#ifdef ONE_FACTORY
    //TPD_DMESG(" [7c16]=0x%02x,  [7c17]=0x%02x, [7c18]=0x%02x, [7c19]=0x%02x\n",  file_fw_data[31766], file_fw_data[31767], file_fw_data[31768], file_fw_data[31769]);
    New_FW_ID = file_fw_data[0x8729] << 8  | file_fw_data[0x8728] ;
    New_FW_VER = file_fw_data[0x850B] << 8  | file_fw_data[0x850a] ;
	TPD_DMESG(" [7c16]=0x%02x,  [7c17]=0x%02x, [7c18]=0x%02x, [7c19]=0x%02x\n", 
	file_fw_data[0x8729], file_fw_data[0x8728], file_fw_data[0x850B], file_fw_data[0x850A]);
#endif

        /* for ektf31xx iap ekt file   */
        /* 
        TPD_DMESG(" [7bd8]=0x%02x,  [7bd9]=0x%02x, [7bda]=0x%02x, [7bdb]=0x%02x\n",  file_fw_data[31704],file_fw_data[31705],file_fw_data[31706],file_fw_data[31707]);
        New_FW_ID = file_fw_data[31707]<<8  | file_fw_data[31708] ;
        New_FW_VER = file_fw_data[31705]<<8  | file_fw_data[31704] ;*/
        TPD_DMESG(" FW_ID=0x%x,   New_FW_ID=0x%x \n",  FW_ID, New_FW_ID);
        TPD_DMESG(" FW_VERSION=0x%x,   New_FW_VER=0x%x \n",  FW_VERSION  , New_FW_VER);
        
        /* for firmware auto-upgrade*/
        if (New_FW_ID   ==  FW_ID){
        	if (New_FW_VER > (FW_VERSION))
        				Update_FW_in_Driver(client);
        } else {
        				TPD_DMESG("FW_ID is different!");
        }
        /*enter recovery*/
        if (FW_ID == 0)  
                RECOVERY=0x80;
    ...
    TPD_DMESG("[elan] %s: ts->LCM_ID0 = %d\n", __func__, ts->LCM_ID0);
    TPD_DMESG("[elan] %s: ts->LCM_ID2 = %d\n", __func__, ts->LCM_ID2);
    TPD_DMESG("[elan] %s: FW_ID = 0x%02x\n", __func__, FW_ID);
    TPD_DMESG("[elan] %s: FW_VERSION = 0x%02x\n", __func__, FW_VERSION);
    //TPD_DMESG("[elan] %s: build_version = %d\n", __func__, build_version);

    //if(build_version == 1)
    if(1)
        TPD_DMESG("%s: Skip touch FW update in eng mode ...\n", __func__);
    else
    {
        if(ts->LCM_ID2) // AUO LCM
        {
            ...
            {
                TPD_DMESG("[elan] %s: Start touch FW update to version 0x3038_0x551f for AUO LCM...\n", __func__);
                ...
                TPD_DMESG("[elan] %s: FW Check Pass !\n", __func__);
        }
        else // CPT LCM
        {
            ...
            {
                TPD_DMESG("[elan] %s: Start touch FW update to version 0x3037_0x554c for CPT LCM...\n", __func__);
                //elan_TWO_WIRE_ICE(ts->client);
                Update_FW_in_Driver(0);
    ...
    TPD_DMESG("[elan] %s: register switch device ...\n", __func__); //  for testtest
    ...
    {
        TPD_DMESG("%s: switch_dev_register failed!\n", __func__);
    }
    //  for testtest --
    ...
    {
        TPD_DMESG("[elan]  failed to create kernel thread\n");
    }
#endif

    TPD_DMESG("[elan] %s: create_proc_entry ...\n", __func__); //  for testtest
#ifdef _ENABLE_DBG_LEVEL
...
static int elan_ktf_ts_set_power_state(struct i2c_client *client, int state)
{
    ...
    TPD_DMESG("[elan] dump cmd: %02x, %02x, %02x, %02x\n", cmd[0], cmd[1], cmd[2], cmd[3]);
...
static int elan_ktf_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
    ...
    if(power_lock == 0) /* The power_lock can be removed when firmware upgrade procedure will not be enter into suspend mode.  */
    {
        ...
        if(ts->gesture & GESTURE_ENABLE)
        {
            ...
            if(ret < 0)
                TPD_DMESG("[elan] %s: send gesture_mode_check_cmd fail\n", __func__);

            rc = i2c_master_recv(client, buf_recv, 4);
            if(rc != 4)
                TPD_DMESG("[elan] %s: read gesture mode state error.\n", __func__);

            TPD_DMESG("[elan] %s: gesture mode state: %2x:%2x:%2x:%2x\n", __func__, buf_recv[0], buf_recv[1], buf_recv[2], buf_recv[3]);
            //  for hello package in gesture mode --
        }
        else
        {
            TPD_DMESG(KERN_INFO "[elan] %s: enter sleep mode\n", __func__);
...
static int elan_ktf_ts_resume(struct i2c_client *client)
{
    ...
    if(power_lock == 0) /* The power_lock can be removed when firmware upgrade procedure will not be enter into suspend mode.  */
    {
        ts->system_suspend = 0; //  for hello package in gesture mode ++
        TPD_DMESG(KERN_INFO "[elan] %s: enter\n", __func__);
#ifdef ESD_CHECK
        schedule_delayed_work(&private_ts->check_work, msecs_to_jiffies(2500));
#endif
#ifdef ELAN_RESUME_RST
        TPD_DMESG("[elan] %s: Used Reset instead of command to resume touch panel\n", __func__);
        elan_ktf_ts_hw_reset();
...
#ifdef RE_CALIBRATION
            rc = i2c_master_recv(client, buf_recv, 4);
            TPD_DMESG("[elan] %s: Re-Calibration Packet %2x:%2x:%2x:%2x\n", __func__, buf_recv[0], buf_recv[1], buf_recv[2], buf_recv[3]);
            if(buf_recv[0] != 0x66)
            {
                mdelay(200);
                rc = i2c_master_recv(client, buf_recv, 4);
                TPD_DMESG("[elan] %s: Re-Calibration Packet, re-try again %2x:%2x:%2x:%2x\n", __func__, buf_recv[0], buf_recv[1], buf_recv[2], buf_recv[3]);
            }
#endif
            rc = elan_ktf_ts_get_power_state(client);
            if(rc != PWR_STATE_NORMAL)
                TPD_DMESG(KERN_ERR "[elan] %s: wake up tp failed! err = %d\n",
                          __func__, rc);
            else
                break;
        }
...
static int elan_i2c_suspend(struct i2c_client *client, pm_message_t mesg)
{
		suspend_gesture = true;
		tpd_halt = 1;
		//printk("[elan] %s  finish\n",__func__);
    return 0;
}

static int elan_i2c_resume(struct i2c_client *client)
{
		suspend_gesture = false;
		tpd_halt = 0;
		wake_up_interruptible(&waiter_resume);
		//printk("[elan] %s wake_up_interruptible waiter_resume\n",__func__);
    return 0;
}
static struct i2c_driver ektf_ts_driver =
{
    ...
    .suspend	= elan_i2c_suspend,
    .resume		= elan_i2c_resume,
...
*/
   struct elan_ktf_ts_data *ts = private_ts;
   int ret =0;
  uint8_t gesture_wake_up_cmd[] = {0x54, 0x58, 0x0f, 0x01};

#if ACER_GESTURE_WAKEUP
	if(mIsEnableGestureWakeUp)
	{	
        touch_debug(DEBUG_INFO,"-->>hxh tpd_suspend acer Gesture  WakeUp on.");
		return;
	}
#endif  
   touch_debug(DEBUG_INFO,"-->>hxh  tpd_suspend acer Gesture  WakeUp off.");
   ret = i2c_master_send(ts->client, gesture_wake_up_cmd, sizeof(gesture_wake_up_cmd));
            if(ret < 0)
                TPD_DMESG("[elan] %s: send gesture_wake_up_cmd fail\n", __func__);
    gpio_direction_output(ts->rst_gpio, 0);
...
//	tp_vgp_supply_disable();
   ...
   // disable_irq(private_ts->intr_gpio);
...
static void tpd_resume(struct device *h)
{
	...
	{
		// Gesture WakeUp on
		touch_debug(DEBUG_INFO,"hxh tpd_resume acer Gesture WakeUp on.");
		return;
	}
	else
	{
		// Gesture WakeUp off
		touch_debug(DEBUG_INFO,"hxh tpd_resume acer Gesture WakeUp off.");
	}
#endif	
...
//	tp_vgp_supply_enable();
   ...
   //enable_irq(private_ts->intr_gpio);
