...
CHARGER_TYPE usb_charger_type_detect(void)
{
	...
	printk("======PHY:0x68 = 0x%x\n",USBPHY_READ8(0x68));
	line_state = USBPHY_READ8(0x76) >> 6;
	printk("==================usb_charger_type_detect:0x76 = 0x%x\n",USBPHY_READ8(0x76));
	printk("==================usb_charger_type_detect:line_state = 0x%x\n",line_state);

	switch(line_state) {
	case 0x0:
	case 0x1:
		printk("************charger_type = Host **********\n");
	...
	case 0x2:
	case 0x3:
		printk("======enter adapter type detect ========\n");
		//musb_set_soft_conn(false);
		USBPHY_SET8(0x6a,0x10);
		mdelay(100);
		USBPHY_SET8(0x6a,0x30);
		...
		line_state = USBPHY_READ8(0x76) >> 6;
		printk("======adapter type: 0x76 = 0x%x========\n",USBPHY_READ8(0x76));
		if (line_state == 0x1) {
			printk("************charger_type = Non-standard **********\n");
			CHR_Type_num = NONSTANDARD_CHARGER;
		}
		else {
			printk("************charger_type = Standard **********\n");
