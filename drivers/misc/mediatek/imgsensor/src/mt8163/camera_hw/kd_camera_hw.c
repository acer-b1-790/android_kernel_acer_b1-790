...
#define VOL_1000 1000000
	...
	if (On) {
		...
		//add clj 20160121
		 if ((pinSetIdx==0) && currSensorName && (0 == strcmp(SENSOR_DRVNAME_GC2145_YUV, currSensorName)))
   		 {
				 ...
				 printk("clj5[PowerON]pinSetIdx:%d, currSensorName: %s\n", pinSetIdx, currSensorName);
               ...
               mdelay(1);
				...
				//enable active sensor
				 ...
				 printk("clj6[PowerON]pinSetIdx:%d, currSensorName: %s\n", pinSetIdx, currSensorName);
		 ...
		 //add clj 20150413
		...
		else if ((pinSetIdx==1) && currSensorName && (0 == strcmp(SENSOR_DRVNAME_GC0328_YUV, currSensorName)))
   		 {
				 ...
				 printk("clj4[PowerON]pinSetIdx:%d, currSensorName: %s\n", pinSetIdx, currSensorName);
  		...
  		else if (currSensorName && pinSetIdx == 0 && (0 == strcmp(currSensorName, SENSOR_DRVNAME_OV5670_MIPI_RAW)))
       {
		 ...
		 PK_DBG("clj2[PowerON]pinSetIdx:%d, currSensorName: %s\n", pinSetIdx, currSensorName);

	  if (GPIO_CAMERA_INVALID != pinSet[1][IDX_PS_CMPDN])
		  mtkcam_gpio_set(1, CAMPDN, pinSet[1][IDX_PS_CMPDN + IDX_PS_ON]);  //0
	  
	  if (GPIO_CAMERA_INVALID != pinSet[1][IDX_PS_CMRST])
		  mtkcam_gpio_set(1, CAMRST, pinSet[1][IDX_PS_CMRST + IDX_PS_OFF]);  //0
      ...
      mdelay(15);
		...
		if (pinSetIdx == 0 && currSensorName && (0 == strcmp(currSensorName, SENSOR_DRVNAME_OV5648_MIPI_RAW))) {
			...
			else if(pinSetIdx == 0 && currSensorName && (0 == strcmp(currSensorName, SENSOR_DRVNAME_GC2145_YUV))){
				printk("cljSENSOR_DRVNAME_GC2145_YUV_poweroff\n ");
                mdelay(10);
			...
			/* VCAM_IO */
			if (TRUE != _hwPowerDownCnt(pinSetIdx, VCAMA, mode_name)) {
				printk("clj#1 gc2145[CAMERA SENSOR] Fail to OFF digital power (VCAM_IO), power id = %d\n", VCAMIO);
				/* return -EIO; */
				goto _kdCISModulePowerOn_exit_;
				}
             
			 if (TRUE != _hwPowerDownCnt(pinSetIdx, VCAMD, mode_name)) {
				printk("clj#2gc2145[CAMERA SENSOR] Fail to OFF digital power (VCAM_IO), power id = %d\n", VCAMIO);
				/* return -EIO; */
				goto _kdCISModulePowerOn_exit_;
			 }
			...
			if (TRUE != _hwPowerDownCnt(pinSetIdx, VCAMIO, mode_name)) {
			    printk("clj#3gc2145[CAMERA SENSOR] Fail to OFF digital power (VCAM_IO), power id = %d\n", VCAMIO);
			    /* return -EIO; */
			    goto _kdCISModulePowerOn_exit_;
	            }
			...
			printk("cljSENSOR_DRVNAME_GC0312_YUV_poweroff\n ");
			...
			/* VCAM_IO */
			if (TRUE != _hwPowerDownCnt(pinSetIdx, VCAMA, mode_name)) {
				printk("clj#1 gc0312[CAMERA SENSOR] Fail to OFF digital power (VCAM_IO), power id = %d\n", VCAMIO);
				/* return -EIO; */
				goto _kdCISModulePowerOn_exit_;
				}

			if (TRUE != _hwPowerDownCnt(pinSetIdx, VCAMD, mode_name)) {
			   printk("clj#2gc0312[CAMERA SENSOR] Fail to OFF digital power (VCAM_IO), power id = %d\n", VCAMIO);
			   /* return -EIO; */
			   goto _kdCISModulePowerOn_exit_;
			}

			if (TRUE != _hwPowerDownCnt(pinSetIdx, VCAMIO, mode_name)) {
			    printk("clj#3gc0312[CAMERA SENSOR] Fail to OFF digital power (VCAM_IO), power id = %d\n", VCAMIO);
			    /* return -EIO; */
			    goto _kdCISModulePowerOn_exit_;
	            }
			}
				...
				else if(pinSetIdx == 1 && currSensorName && (0 == strcmp(currSensorName, SENSOR_DRVNAME_GC0328_YUV))) {
			printk("cljSENSOR_DRVNAME_GC0312_YUV_poweroff\n ");
