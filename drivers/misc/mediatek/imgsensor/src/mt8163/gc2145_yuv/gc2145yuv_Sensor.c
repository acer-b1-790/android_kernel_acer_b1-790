		...
		if (GC2145_sensor_cap_state == KAL_FALSE) 
		{
			if (enable) 
			{
				if (GC2145_VEDIO_encode_mode == KAL_TRUE) 
				{
					...
					GC2145_write_cmos_sensor(0x3c, 0x70);//0x60
					GC2145_write_cmos_sensor(0xfe, 0x00);
				}
				else 
				{
			...
			else 
			{
				...
				{
					GC2145_write_cmos_sensor(0xfe, 0x01);
					GC2145_write_cmos_sensor(0x3c, 0x60);//0x40
					GC2145_write_cmos_sensor(0xfe, 0x00);
				}
		}
	}
...
static void GC2145_Sensor_Init(void)
{
	...
	GC2145_write_cmos_sensor(0x17 , 0x14); //mirror 14  20160520 clj
	...
	GC2145_write_cmos_sensor(0x24 , 0x1E);
	...
	GC2145_write_cmos_sensor(0x85 , 0x0c); //0x08
	...
	GC2145_write_cmos_sensor(0x43 , 0x5f);//0x5b 
  ...
  GC2145_write_cmos_sensor(0xdb,0x3c);// left_down_b22 of B	
	...
	GC2145_write_cmos_sensor(0x61 , 0xdb);//0xdb 
	GC2145_write_cmos_sensor(0x62 , 0xb8);//0xb8
	...
	///////////////dark sun////////////////////
	GC2145_write_cmos_sensor(0xfe , 0x02);
	GC2145_write_cmos_sensor(0x40 , 0x3f);//0xbf 
...
BOOL GC2145_set_param_wb(UINT16 para)
{
	switch (para)
	{
		...
		case AWB_MODE_INCANDESCENT: //office
			...
			GC2145_write_cmos_sensor(0xb4, 0x40);
			GC2145_write_cmos_sensor(0xb5, 0x60);//0xa8
		break;
		case AWB_MODE_TUNGSTEN: //home
			GC2145_set_AWB_mode(KAL_FALSE);
			GC2145_write_cmos_sensor(0xb3, 0x90);
		...
		case AWB_MODE_FLUORESCENT:
			...
			GC2145_write_cmos_sensor(0xb4, 0x60);//0x40
...
UINT32 GC2145GetDefaultFramerateByScenario(MSDK_SCENARIO_ID_ENUM scenarioId, MUINT32 *pframeRate) 
{
 ...
 void GC2145_get_AEAWB_lock(unsigned long long *pAElockRet32, unsigned long long *pAWBlockRet32)
 {
							 ...
							 UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    //UINT32 *feature_data=(UINT32 *) pFeaturePara;
   unsigned long long *feature_data=(unsigned long long *) pFeaturePara;
	...
	switch (FeatureId)
	{
		...
		case SENSOR_FEATURE_SET_ISP_MASTER_CLOCK_FREQ:
			//modify clj 20150730
			GC2145_isp_master_clock=*feature_data;
		...
		case SENSOR_FEATURE_GET_AE_AWB_LOCK_INFO:
			GC2145_get_AEAWB_lock((unsigned long long *)(*feature_data),(unsigned long long *) (*(feature_data + 1))); //yangce added for AE_lock CTS
			break;

		case SENSOR_FEATURE_CHECK_SENSOR_ID:
			
			GC2145_GetSensorID(pFeatureReturnPara32);
		...
		GC2145YUVSensorSetting((FEATURE_ID)*feature_data, *(feature_data+1));  //modify clj 20150410 
